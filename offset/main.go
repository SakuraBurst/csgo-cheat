...
var Signatures *SSignatures
var Netvars *SNetvars

func init() {
	res, err := http.Get("https://raw.githubusercontent.com/frk1/hazedumper/master/csgo.json")
	if err != nil {
		logger.ErrorLogger.Fatalln("An error occured while fetching offets:", chalk.Red.Color(err.Error()))
	}

	body, err := ioutil.ReadAll(res.Body)
	if err != nil {
		logger.ErrorLogger.Fatalln("An error occured while marshalling offsets:", chalk.Red.Color(err.Error()))
	}

	var offsets SOffsets
	err = json.Unmarshal(body, &offsets)
	if err != nil {
		logger.ErrorLogger.Fatalln("An error occured while Unmarshalling offsets:", chalk.Red.Color(err.Error()))
	}

	if offsetsOutdated(offsets) {
		logger.WarningLogger.Println("Offsets possibly", chalk.Yellow.Color("OUTDATED"))
	}
...
func offsetsOutdated(Offsets SOffsets) bool {
	...
	if err != nil {
		logger.ErrorLogger.Fatalln("An error occured while parsing time:", chalk.Red.Color(err.Error()))
	}
