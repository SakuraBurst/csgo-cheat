...
func ProcessInfo(pid uint32) (Process, error) {
	...
	if w32.Module32First(snap, &me) {
		...
		for w32.Module32Next(snap, &me) {
			if proc.Name == "csgo.exe" {
				fmt.Println(w32.UTF16PtrToString(&me.SzModule[0]))
			}
...
func ProcessByName(name string) (Process, error) {
	...
	snap = w32.CreateToolhelp32Snapshot(syscall.TH32CS_SNAPALL, 0)
	pe32.Size = uint32(unsafe.Sizeof(pe32))
	err := w32.Process32First(snap, &pe32)
	if err != nil {
		w32.CloseHandle(snap)
		return Process{}, fmt.Errorf("process %s not found", name)
	} else {
		for i := err; i == nil; i = w32.Process32Next(snap, &pe32) {
			processInfo, err := ProcessInfo(pe32.ProcessID)
			fmt.Println(processInfo.Name)
